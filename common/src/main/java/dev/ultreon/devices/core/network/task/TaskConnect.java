...
public class TaskConnect extends Task {
    ...
    public void processRequest(CompoundTag tag, Level level, Player player) {
        BlockEntity tileEntity = level.getChunkAt(BlockPos.of(tag.getLong("routerPos").orElseThrow())).getBlockEntity(BlockPos.of(tag.getLong("routerPos").orElseThrow()), LevelChunk.EntityCreationType.IMMEDIATE);
        if (tileEntity instanceof RouterBlockEntity tileEntityRouter) {
            Router router = tileEntityRouter.getRouter();

            BlockEntity tileEntity1 = level.getChunkAt(BlockPos.of(tag.getLong("devicePos").orElseThrow())).getBlockEntity(BlockPos.of(tag.getLong("devicePos").orElseThrow()), LevelChunk.EntityCreationType.IMMEDIATE);
