...
public class HeaderModule extends Module {
    ...
    public int calculateHeight(Map<String, String> data, int width) {
        if (data.containsKey("scale")) {
            return (int) (Double.parseDouble(data.get("scale")) * Laptop.getLaptopFont().lineHeight + (data.containsKey("padding") ? Integer.parseInt(data.get("padding")) : 5) * 2);
        }
        return Laptop.getLaptopFont().lineHeight + (data.containsKey("padding") ? Integer.parseInt(data.get("padding")) : 5) * 2;
    }
